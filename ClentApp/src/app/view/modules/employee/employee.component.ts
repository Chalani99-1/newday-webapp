...
export class EmployeeComponent {
  ...
  initialize() {
    ...
    this.gs.getAllList().then((gends: Gender[]) => {
      this.genders = gends;
    });
    this.ds.getAllList().then((ds: Designation[]) => {
      this.designations = ds;
    });
    this.et.getAllList().then((empt: Emptype[]) => {
      this.employeetypes = empt;
    });
    this.ss.getAllList().then((emps: Empstatus[]) => {
      this.employeestatuses = emps;
    });
