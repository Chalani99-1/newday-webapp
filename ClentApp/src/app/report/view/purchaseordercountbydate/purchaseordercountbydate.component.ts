...
@Component({
  selector: 'app-purchaseordercountbydate',
  templateUrl: './purchaseordercountbydate.component.html',
  styleUrls: ['./purchaseordercountbydate.component.css']
})
export class PurchaseordercountbydateComponent {

  maxDate: Date = new Date();  // Today's date
  minDate = new Date(new Date(this.maxDate).setDate(this.maxDate.getDate()+1));

  purchaseordercountbydates!: Purchaseordercountbydate[];
  cntbysupdata!: MatTableDataSource<Purchaseordercountbydate>;

  public supsearch!: FormGroup;
  searchEnable = false;
  stdate = '';
  endate = '';
  columns: string[] = ['supplierName', 'count', 'percentage'];
  headers: string[] = ['Supplier Name', 'Purchase Order Count', 'Percentage'];
  binders: string[] = ['supplierName', 'count', 'percentage'];
      ...
      tooltip: {
        callbacks: {
          label: (tooltipItem) => {
            ...
            const percentage = this.purchaseordercountbydates?.[dataIndex]?.percentage ?? '0';
  ...
  loadSupTableInit(query: string) {
    this.rs.purchaseordercountbydateAll()
      .then((pocs: Purchaseordercountbydate[]) => {
        this.purchaseordercountbydates = pocs;
        this.amounts = this.purchaseordercountbydates.map(c => ({
          'count': c.count, 'percentage': c.percentage
        }))
        this.cntbysupdata = new MatTableDataSource(this.purchaseordercountbydates);
        this.loadChartData();
      }).finally(() => {
  ...
  loadSupTable(query: string) {
    this.rs.purchaseordercountbydate(query)
      .then((pocs: Purchaseordercountbydate[]) => {
        this.purchaseordercountbydates = pocs;
        this.cntbysupdata = new MatTableDataSource(this.purchaseordercountbydates);
        this.loadChartData();
  ...
  loadChartData(): void {
    this.pieChartData.labels = this.purchaseordercountbydates
      .map(item => item.supplierName);
    this.pieChartData.datasets[0].data = this.purchaseordercountbydates
      .map(item => item.count);
  ...
  btnsupSearchMC() {
    ...
    const supsearchdata = this.supsearch.getRawValue();
    let startdate = supsearchdata.startdate;
    let enddate = supsearchdata.enddate;
    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; padding: 10px; max-width: 1200px; margin: auto;">
      ...
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="margin: 0; font-size: 20px; color: #0d6efd; border-bottom: 2px solid #0d6efd; display: inline-block; padding-bottom: 5px;">
          Supplier-wise Purchase Order Count Table
        </h1>
      </div>
 ...
 <!-- Custom separator line -->
  ...
  <div style="page-break-before: always; text-align: center; margin: 30px 0;">
      <h1 style="margin: 0; font-size: 20px; color: #0d6efd; border-bottom: 2px solid #0d6efd; display: inline-block; padding-bottom: 5px;">
         Supplier-wise Purchase Order Count Chart
      </h1>
    </div>
  ...
  `;
    ...
    const opt = {
      margin: 0.5,
      filename: 'supplier_po_count_report.pdf',
